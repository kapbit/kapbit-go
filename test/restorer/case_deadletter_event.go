...
func DeadLetterEventTestCase() TestCase {
	name := "Should correctly handle DeadLetterEvent during restoration"
	...
	store.RegisterLoadRecent(
		func(ctx context.Context, partition, workflowsCount int,
			...
			return eventFn(1, false, &evnt.DeadLetterEvent{
				Key: wid,
			})
		},
	...
	return TestCase{
		...
		Want: Want{
			CounterValue:         0,
			IdempotencyWindowStr: "support.FIFOSet cap=1000 len=1 [wfl-dead]",
