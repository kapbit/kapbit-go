...
func TwoCreateTestCase() TestCase {
	name := "Should be able to handle only two created events"
	...
	var (
		...
		event2 = evnt.WorkflowCreatedEvent{
			NodeID:    "node-1",
			Key:       wfl.ID("wfl-2"),
	...
	return TestCase{
		...
		Want: Want{
			...
			Check: func(t *testing.T, config TestSetup) {
				...
				asserterror.Equal(t, receivedChpt, 0)
				...
				asserterror.Equal(
					t,
					strings.HasSuffix(
						tracker.String(), "partitions=1 [p0:NextN=2] tracked=[wfl-1:{0, 0, 10} wfl-2:{0, 1, 11}]",
					),
					true,
				)
			},
