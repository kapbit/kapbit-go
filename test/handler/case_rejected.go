...
func RejectedTestCase() TestCase {
	name := "Should add checkpoint and untrack on terminal rejected event"
	...
	var (
		...
		event2 = &evnt.WorkflowRejectedEvent{
			NodeID:    "node-1",
			Key:       wfl.ID("wfl-1"),
			Type:      wfl.Type("type-a"),
			Timestamp: 1,
	...
	return TestCase{
		...
		Want: Want{
			...
			Check: func(t *testing.T, config TestSetup) {
				rejectedEvent := event2
				rejectedEvent.WorkflowOffset = 10
				...
				asserterror.EqualDeep(t, receivedEvent2, rejectedEvent)
